...
function renderProductChart() {
    const t = window.translations?.admin || {};

    if (!window.chartData || !Array.isArray(window.chartData.stats) || window.chartData.stats.length === 0) {
        console.warn(t.no_chart_data || "Немає даних для productChart");
        return;
    }
    ...
    const productData = {
        ...
        datasets: [{
            label: t.request_count || "Кількість заявок",
...
function renderComplexityChart() {
    ...
    if (!window.chartData || !window.chartData.complexity_stats || window.chartData.complexity_stats.length === 0) {
        console.warn(t.no_chart_data || "Немає даних для complexityChart");
        return;
    }
    ...
    const complexityData = {
        ...
        datasets: [{
            label: t.complexity_distribution || "Розподіл складності",
